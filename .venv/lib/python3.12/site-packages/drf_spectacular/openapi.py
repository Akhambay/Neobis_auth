...
class AutoSchema(ViewInspector):
    ...
    def _process_override_parameters(self):
        ...
        for parameter in self.get_override_parameters():
            if isinstance(parameter, OpenApiParameter):
                ...
                warn(f'could not resolve parameter annotation {
                     parameter}. Skipping.')
        return result
    ...
    def get_auth(self):
        ...
        for authenticator in self.view.get_authenticators():
            ...
            if not scheme:
                warn(
                    f'could not resolve authenticator {
                        authenticator.__class__}. There '
    ...
    def _resolve_path_parameters(self, variables):
        ...
        for variable in variables:
            ...
            if resolved_parameter:
                ...
                warn(
                    f'could not derive type of path parameter "{
                        variable}" because it '
                    f'is untyped and obtaining queryset from the viewset failed. '
                    f'Consider adding a type to the path (e.g. <int:{
                        variable}>) or annotating '
                ...
                try:
                    ...
                    warn(
                        f'could not derive type of path parameter "{
                            variable}" because model '
                        f'"{model.__module__}.{
                            model.__name__}" contained no such field. Consider '
    ...
    def _map_model_field(self, model_field, direction):
        ...
        if field and isinstance(field, serializers.PrimaryKeyRelatedField):
            ...
            if not field_cls:
                warn(
                    f'model field "{
                        model_field.get_internal_type()}" has no mapping in '
            ...
            error(
                f'could not resolve model field "{
                    model_field}". Failed to resolve through '
    ...
    def _map_serializer_field(self, field, direction, bypass_extensions=False):
            ...
            if method is None:
                error(
                    f'SerializerMethodField "{
                        field.field_name}" is missing required method '
        ...
        if isinstance(field, serializers.ReadOnlyField):
            ...
            if model is None:
                warn(
                    f'Could not derive type for ReadOnlyField "{
                        field.field_name}" because the '
            ...
            if callable(target):
                ...
                assert False, f'ReadOnlyField target "{
                    field}" must be property or model field'
            return append_meta(schema, meta)
        ...
        warn(f'could not resolve serializer field "{
             field}". Defaulting to "string"')
        return append_meta(build_basic_type(OpenApiTypes.STR), meta)
    ...
    def _map_response_type_hint(self, method):
        ...
        try:
            ...
            warn(
                f'unable to resolve type hint for function "{
                    method.__name__}". Consider '
        ...
        if isinstance(request_serializer, dict):
            ...
            content = [
                (media_type, schema, self._get_examples(
                    request_serializer, 'request', media_type))
    ...
    def _get_request_for_media_type(self, serializer):
        ...
        if is_list_serializer(serializer):
            ...
            warn(
                f'could not resolve request body for {self.method} {
                    self.path}. Defaulting to generic '
    ...
    def _get_response_bodies(self):
        ...
        if (
            ...
            warn(
                f'could not resolve "{response_serializers}" for {
    ...
    def _get_response_for_code(self, serializer, status_code, media_types=None):
        ...
        if not serializer:
            ...
            warn(
                f'could not resolve "{serializer}" for {
                    self.method} {self.path}. Expected either '
            ...
            **headers,
            'content': {
                media_type: build_media_type_object(
                    schema,
                    self._get_examples(serializer, 'response',
    ...
    def _get_response_headers_for_code(self, status_code) -> dict:
        ...
        for parameter in self.get_override_parameters():
            ...
            if is_basic_type(parameter.type):
                ...
                schema = self.resolve_serializer(
                    parameter.type, 'response').ref
            else:
                schema = parameter.type

            if parameter.location not in [OpenApiParameter.HEADER, OpenApiParameter.COOKIE]:
                warn(f'incompatible location type ignored for response parameter {
                     parameter.name}')
